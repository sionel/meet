...
const ConferenceScreenPresenter = props => {
  // 선택된 사람
  const selectedPerson = props.participant.filter(
    p => p.id === props.selected
  )[0];
  // 선택 스타일
  ...
  return (
    <View style={{ ...styles.container }}>
      <ImageBackground
        ...
        source={
          selectedPerson.conferenceStatus.camera
            ? { uri: selectedPerson.img, width: 100 }
            : buttonCameraOff
        }
      >
        {/* 상단영역 */}
            ...
            {props.participant.map((person, personKey) => (
              <View
                style={{
                  ...styles.participant,
                  marginRight:
                    personKey === props.participant.length - 1 ? 10 : 0
                }}
                key={person.id}
              >
                <TouchableHighlight
            ...
            <View style={{ ...styles.buttonsArea }}>
              {bottomControl.map(btn => (
                <View style={{ ...styles.button }} key={btn.name}>
                  <TouchableHighlight
                    onPress={() => props.onToggleStatus(btn.name)}
                  >
                    <Image
                      source={btn.button}
