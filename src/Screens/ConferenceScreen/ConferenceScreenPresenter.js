...
const ConferenceScreenPresenter = (props) => {
  ...
  <View style={{...styles.container}}>
    <ImageBackground 
      ...
      source={(selectedPerson.conferenceStatus.camera) ? {uri: selectedPerson.img, width:100} : buttonCameraOff}
    >
      {/* 상단영역 */}
      <View style={{...styles.topArea, ...styles.commonStyle, justifyContent: "flex-end", alignItems: "flex-end"}}>
        {/* 경과시간 */}
        <View style={{flex:0.5}}>
      ...
      <View style={{...styles.middleArea, ...styles.commonStyle}}>
        <TouchableHighlight 
          style={styles.fullSizeLayout} 
          onPress={()=>props.onScreenTouch()}
        >
          <View></View>
        </TouchableHighlight>
      </View>
                  ...
                  <Fragment>
                    {
                      (person.conferenceStatus.camera) ? (
                        ...
                        <Image 
                          style={{width:45, height:45, marginLeft:30, marginTop:30, ...selectedStyle(person.id)}}
