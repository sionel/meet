...
const ParticipantBoxPresenter = props => {
	...
	const content =
		stream && !props.isMuteVideo ? (
			<RTCView style={styles.video} mirror={false} objectFit={'cover'} streamURL={stream.toURL()} />
		) : (
	...
	return (
		<TouchableOpacity style={styles.container} onPressOut={() => props.setMainUser(props.user.id)}>
			<View style={props.isSelect ? styles.videoAreaSelected : styles.videoArea}>{content}</View>
