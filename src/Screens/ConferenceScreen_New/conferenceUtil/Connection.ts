...
const Connection = (roomId: string, token: string) => {
  const connect = async () => {
    return new Promise((resolve, reject) => {
      // jitsi connection 을 생성한다.
      const jitsiConnection = _creaeteJitsiConnection(roomId, token);
      // 이벤트를 바인딩한다. -> 바인딩된 이벤트가 호출되어야지 프라미스가 종료된다.
      bindEvents(jitsiConnection, resolve, reject);
      // 커넥션을 연결한다.
      this._jitsiConnection.connect({});
    });
  };

  const _creaeteJitsiConnection = (roomId: string, token: string) => {
