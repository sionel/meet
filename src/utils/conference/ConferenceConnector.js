...
class ConferenceConnector {
   ...
   */
	_createRoom = (connection, roomName) => {
		...
		const room = connection.jitsiConnection.initJitsiConference(roomName, options);
		return room;
	};
   ...
   */
	_bindEvents = (resolve, reject) => {
       ...
       */
		...
		// JOIN_USER 이벤트 연결
		this._room.on(conferenceEvents.USER_JOINED, (id, user) => this._handlers.JOIN_USER(user));

		// JOIN_USER 이벤트 연결
		this._room.on(conferenceEvents.USER_LEFT, id => this._handlers.LEFT_USER(id));
		...
		// SUSPEND_DETECTED
		this._room.on(conferenceEvents.SUSPEND_DETECTED, () => this._handlers.SUSPEND_DETECTED());
     ...
     */
		this._room.addCommandListener(SET_DOCUMENT_IS_SHARE, value => {
			const { value: userId, attributes: { isDocumentShare } } = value;
     ...
     */
		this._room.addCommandListener(UPDATE_DRAWING_DATA, value => {
			...
			if (userId !== this._room.myUserId()) {
				...
				console.log('Draw Data : ', _drawData);
