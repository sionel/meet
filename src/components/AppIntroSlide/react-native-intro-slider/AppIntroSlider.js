...
export default class AppIntroSlider extends React.Component {
	...
	_renderItem = item => {
		...
		const bottomSpacer =
			(this.props.bottomButton ? (this.props.showSkipButton ? 44 : 0) + 44 : 0) + (isIphoneX ? 34 : 0) + 64;
	...
	_renderButton = (name, onPress) => {
		const show =
			name === 'Skip' || name === 'Prev' ? this.props[`show${name}Button`] : !this.props[`hide${name}Button`];
		...
		if (this.props.bottomButton) {
			content = (
				<View
					style={[
						styles.bottomButton,
						(name === 'Skip' || name === 'Prev') && { backgroundColor: 'transparent' },
	...
	_onLayout = () => {
		...
		if (width !== this.state.width || height !== this.state.height) {
			...
			const func = () => {
				this.flatList.scrollToOffset({ offset: this.state.activeIndex * width, animated: false });
			};
