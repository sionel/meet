...
export default function Day(props) {
  ...
  if (
    ...
    if (dateOutOfRange) {
      ...
      const sepValue = props.sepValue.substring(1, 2);
      const diff = thisDay.diff(selectedStartDate, 'hour');

      // console.log(thisDay.diff(selectedStartDate, 'hour'));
          ...
          >
            <Text
              style={[
                ...
                custom.textStyle,
                selectedDayTextStyle,
                sepValue === '0' && { color: diff === -6 ? '#fff' : '#FC5356' },
                sepValue === '6' && { color: diff === -6 ? '#fff' : '#1c90fb' }
              ]}
            >
              {day}
            </Text>
